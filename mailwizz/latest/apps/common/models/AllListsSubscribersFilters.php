...
class AllListsSubscribersFilters extends ListSubscriber
{
    ...
    {
        return array(
            array('customers', '_validateMultipleCustomersSelection'),
    ...
    public function attributeLabels()
    {
        return CMap::mergeArray(parent::attributeLabels(), array(
            'customer_id'    => Yii::t('list_subscribers', 'Customer'),
    ...
    public function getCustomersList()
    {
        ...
        $criteria->addNotInCondition('status', array(Customer::STATUS_PENDING_DELETE, Lists::STATUS_ARCHIVED));

        $customers = Customer::model()->findAll($criteria);
        foreach ($customers as $customer) {
            $customersList[$customer->customer_id] = $customer->fullName . '(' . $customer->email . ')';
        }
    ...
    public function getListsList()
    {
        ...
        foreach ($lists as $list) {
            $listsList[$list->list_id] = $list->name . '(' . $list->display_name . ')';
        }
    ...
    public function buildSubscribersCriteria($isCount = false)
    {
	    ...
	    if (empty($lists)) {
		    ...
		    $_criteria->addNotInCondition('status', array(Lists::STATUS_PENDING_DELETE, Lists::STATUS_ARCHIVED));
		    $models = Lists::model()->findAll($_criteria);
     ...
     * @param $params
     */
    public function _validateMultipleCustomersSelection($attribute, $params)
    {
        ...
        foreach ($values as $index => $value) {
            if (!Customer::model()->findByPk($value)) {
                $this->addError($attribute, Yii::t('list_subscribers', 'Invalid customer identifier!'));
                break;
            }
        }
    }
