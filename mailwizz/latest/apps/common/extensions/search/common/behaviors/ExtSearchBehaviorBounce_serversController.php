...
class ExtSearchBehaviorBounce_serversController extends CBehavior 
{
	/**
	 * @return array
	 */
	public function searchableActions()
	{
		return array(
			'index' => array(
				'keywords'          => array('bounce server', 'create bounce server', 'server', 'bounce'),
	...
	public function _skip(SearchExtSearchItem $item)
	{
		if (MW_APP_NAME == 'customer') {
			...
			return (int)$customer->getGroupOption('servers.max_bounce_servers', 0) == 0;
		}
	...
	public function _indexChildrenGenerator($term, SearchExtSearchItem $parent = null)
	{
		...
		foreach ($models as $model) {
			...
			$item->url   = Yii::app()->createUrl('bounce_servers/update', array('id' => $model->server_id));
			$item->score++;
			$items[] = $item->fields;
		}
		return $items;
