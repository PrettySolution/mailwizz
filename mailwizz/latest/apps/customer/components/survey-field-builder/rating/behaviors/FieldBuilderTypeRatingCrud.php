...
class FieldBuilderTypeRatingCrud extends CBehavior
{
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = Yii::app()->request->getPost('SurveyFieldRating', array());
        ...
        foreach ($postModels[$typeName] as $index => $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldRating::model()->findByAttributes(array(
