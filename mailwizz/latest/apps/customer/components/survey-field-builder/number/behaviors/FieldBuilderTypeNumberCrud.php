...
class FieldBuilderTypeNumberCrud extends CBehavior
{
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = Yii::app()->request->getPost('SurveyFieldNumber', array());
        ...
        foreach ($postModels[$typeName] as $index => $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldNumber::model()->findByAttributes(array(
