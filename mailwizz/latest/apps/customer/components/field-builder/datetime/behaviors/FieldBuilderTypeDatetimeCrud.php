...
class FieldBuilderTypeDatetimeCrud extends CBehavior
{
    ...
    protected function buildFieldArray($model)
    {
	    ...
	    // 1.8.1
	    $model->onAttributeHelpTexts = array($this, '_addAttributeHelpTexts');
	...
	{
		$tags = implode(', ', array_map(array('CHtml', 'encode'), array_keys(ListField::getDefaultValueTags())));
		$text = Yii::t('list_fields', 'In case this field is not required and you need a default value for it. The format must be "Y-m-d H:i:s". Following tags are recognized: {tags}', array('{tags}' => $tags));
